...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        @foreach($sections as $key => $definition)
            <a href="{{ route('admin.site-content.edit', $key) }}" class="block rounded-lg border border-hub-purple/10 p-5 transition-colors hover:border-ccs-teal-light">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="font-display font-bold mb-1">{{ $definition['label'] }}</h2>
                        <p class="text-sm text-hub-dark/45 max-w-sm">{{ $definition['description'] }}</p>
                    </div>
                    <span class="shrink-0 text-xs text-hub-dark/45">
