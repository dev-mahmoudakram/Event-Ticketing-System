...
@if($faqs->isNotEmpty())
    <section id="faq" class="hub-shell scroll-mt-24 py-6">
        <div class="hub-panel hub-pad">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)] gap-10 lg:gap-16 items-center">
                {{-- The questions used to leave half the panel empty on wide screens. --}}
                <div class="order-last lg:order-first" data-reveal data-reveal-delay="1">
                    @if($faqImage = \App\Support\SiteText::image('faq', 'image'))
                        <img src="{{ $faqImage }}" alt="" loading="lazy" class="w-full max-w-sm mx-auto rounded-3xl">
                    @else
                        @include('home.partials.faq-illustration')
                    @endif
                </div>

                <div class="flex flex-col gap-4" x-data="{ open: null }">
                    @foreach($faqs as $faq)
