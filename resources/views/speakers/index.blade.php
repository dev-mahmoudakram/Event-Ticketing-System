            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach($speakers as $speaker)
                    ...
                    <div data-reveal data-reveal-delay="{{ min($loop->iteration, 5) }}">
                        <div class="group relative aspect-3/4 rounded-2xl border border-white/10 overflow-hidden select-none">
                            @if($speaker->photoUrl())
                                <img src="{{ $speaker->photoUrl() }}" alt="{{ $speakerName }}" class="w-full h-full object-cover transition-transform duration-500 ease-out sm:group-hover:scale-110" loading="lazy">
                            @else
                            ...
                            @endif
                            {{-- Desktop only: hover reveals this over the photo. On touch screens
                                 (below sm) the same details render as a plain block underneath the
                                 card instead — see below — since :hover never fires there. --}}
                            <div class="hidden sm:flex absolute inset-0 bg-ccs-black/95 p-8 flex-col justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out">
                        ...
                        <div class="sm:hidden mt-4">
                            <h3 class="font-display font-bold text-lg mb-1">{{ $speakerName }}</h3>
                            <p class="text-ccs-coral font-bold text-xs uppercase tracking-wide mb-3">{{ app()->getLocale() === 'ar' ? $speaker->title_ar : $speaker->title_en }}</p>
                            <div class="ccs-richtext text-sm text-gray-400 leading-relaxed">{!! app()->getLocale() === 'ar' ? $speaker->bio_ar : $speaker->bio_en !!}</div>
                        </div>
                    </div>
                @endforeach
